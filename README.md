...
2. Download the appropriate archive for your system (e.g., `grimoire-1.1.7-linux-amd64.tar.gz` or `grimoire-1.1.7-darwin-arm64.tar.gz`).
...
4. Move the `grimoire` executable to a directory in your system's `PATH` (e.g., `/usr/local/bin` or `~/.local/bin`). You may need to use `sudo` for system-wide locations:
   ```bash
   tar -xzf grimoire-1.1.7-linux-amd64.tar.gz
   cd grimoire-1.1.7-linux-amd64
